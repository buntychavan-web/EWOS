...
/**
 * Resolves the current request's authenticated employee, if any. The employee is derived from the JWT's
 * {@code employeeId} claim — set by {@code JwtAuthenticationFilter} as a request attribute at
 * authentication time — mirroring {@code com.ewos.tenancy.application.TenantContext#homeTenantId()}
 * exactly.
 *
 * <p>Unlike {@code TenantContext.homeTenantId()}, absence is not surfaced as an error here: a caller with
 * no linked employee (or an ambiguous multi-company link, left unresolved at login time) is a normal,
 * common state, not a broken session. Callers that require a linked employee (e.g. {@code GET
 * /employees/me}) decide how to respond to an empty result themselves.
 */
@Component
