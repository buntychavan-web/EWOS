...
/**
 * Role Usage Impact Analysis (Product Owner addition, Sprint 1.4) — split out of {@link RoleService}
 * post-audit (Finding 7: that class had grown too many responsibilities).
 *
 * <p><b>Tenant scoping (Sprint 1.4 audit remediation, Finding 1):</b> a system role like {@code
 * SYSTEM_ADMIN} is visible to every tenant, and can legitimately be held by users across every tenant on
 * the platform. Before this fix, {@code assignedUsers()}/{@code impact()} returned the platform-wide user
 * list and company/department breakdown for such a role to any caller who could merely see it — a
 * cross-tenant PII leak. Both methods now narrow the result to the caller's own tenant via {@link
 * TenantMembershipFilter} whenever the role is a system role; a tenant-scoped custom role needs no such
 * narrowing, since only same-tenant users can ever hold one (enforced by {@code
...
public class RoleImpactService {
    ...
    public RoleImpactResponse impact(UUID id) {
        ...
        int pendingTasks =
                workflowUsageResolver.countPendingTasksForRole(lookup.requireTenantId(), role.getName());
    ...
    /**
     * Tenant-scoped roles are already same-tenant-only by construction — no filtering needed. System
     * roles are narrowed to the caller's own tenant; a caller with no resolved tenant at all sees none
    ...
    private List<User> usersInCallerScope(Role role) {
        ...
        return lookup.currentTenantId()
                .map(
                        tenantId -> {
                            Set<UUID> allIds = all.stream().map(User::getId).collect(Collectors.toSet());
                            Set<UUID> inTenant = tenantMembershipFilter.filterToTenant(allIds, tenantId);
