...
/**
 * Sprint 1.4 audit remediation, Finding 2 — ensures a user has at least one active tenant
 * membership, backfilling to the platform's bootstrap tenant if they have none. Exists specifically
 * for {@link IdentityBootstrap}: {@code V38__tenant_resolution.sql}'s backfill runs once, at
 * migration time, over whatever is already in {@code users} — on a genuinely fresh deployment
 * that's nobody, since the bootstrap admin is created afterward by {@link IdentityBootstrap}, an
 * {@code ApplicationRunner} that runs after Flyway migrations complete. Without this, the bootstrap
 * admin could never resolve a tenant at all.
 *
 ...
 * alone owns {@code user_tenant_memberships} and knows the bootstrap tenant's id.
 */
